...
class leavemodal(Modal,title = 'leave form'):
    ...
    year=TextInput(label='year',style=discord.TextStyle.short,placeholder='title',default=datetime.datetime.now().year,required=True,max_length=4,row=0)
    month=TextInput(label='month',style=discord.TextStyle.short,placeholder='title',default=datetime.datetime.now().month,required=True,max_length=2,row=1)
    day=TextInput(label='day',style=discord.TextStyle.short,placeholder='title',default=datetime.datetime.now().day,required=True,max_length=2,row=2)
    reason=TextInput(label='reason',style=discord.TextStyle.long,placeholder='title',default='None',required=True,max_length=50,row=3)

    ###送出假單
    async def on_submit(self, interaction: discord.Interaction):
        try:
            leavelist.append({'user':str(interaction.user),
                              'reason':str(self.reason),
                              'date':datetime.datetime.strptime(str(self.year)+str(self.month)+str(self.day),"%Y%m%d").strftime("%Y-%m-%d")})
            print(leavelist)
